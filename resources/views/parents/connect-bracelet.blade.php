...
<head>
    ...
    <title>{{ __('Smart Bracelet') }} - Taif</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bracelet-id-badge { display: inline-block; background: #f3f4f6; padding: 8px 15px; border-radius: 10px; font-family: monospace; font-size: 16px; color: #374151; font-weight: bold; margin-bottom: 20px; direction: ltr; /* الـ ID دائما يسار لليمين */}

        /* Form */
        .input-group { margin-bottom: 20px; text-align: start; /* منطقي ليدعم بداية السطر حسب اللغة */ }
...
<body>
    ...
    <div class="mobile-screen">
        <div class="content">
            ...
            <div class="header">
                <a href="{{ route('profile') ?? '#' }}" class="back-btn" aria-label="Back">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
                </a>
                <div class="title">{{ __('Smart Bracelet') }}</div>
            ...
            <div class="status-card">
                @if($child && $child->is_bracelet_connected)
                    ...
                    <div class="status-title">{{ __('Device Connected') }}</div>
                    <div class="status-desc">{{ __("Your child's smart bracelet is actively tracking and sending data.") }}</div>
                    
                    <div class="bracelet-id-badge">ID: {{ $child->bracelet_id }}</div>

                    <form action="{{ route('bracelet.disconnect') }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-disconnect">{{ __('Disconnect Device') }}</button>
                    </form>
                @else
                    ...
                    <div class="status-title">{{ __('No Device Found') }}</div>
                    <div class="status-desc">{{ __('Connect a smart bracelet to start monitoring your child\'s activities and vital signs.') }}</div>

                    <form action="{{ route('bracelet.connect') }}" method="POST">
                        @csrf
                        <div class="input-group">
                            <label class="input-label">{{ __('Bracelet Serial Number (ID)') }}</label>
                            <input type="text" name="bracelet_id" class="input-field" placeholder="{{ __('e.g. TAIF-BR-001') }}" required>
                        </div>
                        <button type="submit" class="btn btn-connect">{{ __('Connect Device') }}</button>
                    </form>
                @endif
            </div>
