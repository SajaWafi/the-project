...
<head>
    ...
    <title>{{ __('Settings - Taif Project') }}</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Cairo', sans-serif; }
        ...
        /* 💡 دعم الاتجاهين للقائمة الجانبية */
        .sidebar { 
            width: 260px; 
            min-height: 100vh; 
        ...
        .nav-link.active { border-inline-start: 4px solid var(--taif-orange); } /* 💡 الاتجاه */
        .nav-link i { margin-inline-end: 15px; width: 20px; text-align: center; font-size: 18px; }

        /* 💡 دعم الاتجاهين للمحتوى الرئيسي */
        .main-content { 
        ...
        /* 💡 أيقونة العين وحقل كلمة المرور */
        .password-wrapper { position: relative; }
        .password-wrapper input { padding-inline-end: 45px; }
        .password-wrapper i { position: absolute; inset-inline-end: 15px; top: 15px; cursor: pointer; color: #a0aec0; font-size: 16px; transition: 0.2s; }
        ...
        .alert-error ul { margin-inline-start: 20px; } /* 💡 دعم الاتجاه */
...
<body>
    ...
    <div class="main-content">
        <div class="header-top">
            <h2>{{ __('Settings') }}</h2>
            <small style="color:#718096;">{{ __('Manage your personal profile and account preferences.') }}</small>
        </div>
        ...
        <div class="settings-wrapper">
            <div class="settings-nav">
                <button class="tab-btn active" onclick="openTab(event, 'profile')">
                    <i class="fas fa-user-edit"></i> {{ __('Profile Details') }}
                </button>
                ...
                <a href="{{ route('lang.switch', app()->getLocale() == 'ar' ? 'en' : 'ar') }}" class="tab-btn" style="color: #2c5282; text-decoration: none;">
                    <i class="fas fa-language"></i> {{ __('Language') }}: {{ app()->getLocale() == 'ar' ? 'English' : 'العربية' }}
                </a>
            ...
            <div class="settings-content">
                ...
                <div id="profile" class="tab-panel active">
                    <h3 class="panel-title"><i class="fas fa-user-circle" style="color: var(--taif-orange);"></i> {{ __('Update Information') }}</h3>
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="email" class="form-input" value="{{ $admin->email }}" required dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        </div>

                        <div class="form-group">
                            <label class="form-label">{{ __('Phone Number') }}</label>
                            <input type="text" name="phone" class="form-input" value="{{ $admin->phone }}" dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        </div>

                        <hr style="border: 0; border-top: 1px solid #edf2f7; margin: 30px 0;">
                        <h4 style="color: #4a5568; margin-bottom: 15px; font-size: 16px;">{{ __('Change Password') }}</h4>

                        <div class="form-group">
                            <label class="form-label">{{ __('Current Password') }}</label>
                            <div class="password-wrapper">
                                <input type="password" name="current_password" id="current_password" class="form-input" placeholder="{{ __('Enter current password') }}" dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                                <i class="fas fa-eye-slash" onclick="togglePassword('current_password', this)"></i>
                            </div>
                        </div>

                        <div class="form-group" style="margin-bottom: 30px;">
                            <label class="form-label">{{ __('New Password') }} <span style="font-weight:normal; color:#a0aec0;">{{ __('(Leave blank to keep current)') }}</span></label>
                            <div class="password-wrapper">
                                <input type="password" name="password" id="new_password" class="form-input" placeholder="{{ __('Enter new password') }}" dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                                <i class="fas fa-eye-slash" onclick="togglePassword('new_password', this)"></i>
                            </div>
                        </div>

                        <button type="submit" class="btn-save"><i class="fas fa-save me-2"></i> {{ __('Save Changes') }}</button>
                    </form>
                </div>

                <div id="add-admin" class="tab-panel">
                    <h3 class="panel-title"><i class="fas fa-user-plus" style="color: var(--taif-green);"></i> {{ __('Add New Admin') }}</h3>
                    ...
                    <form action="{{ route('admin.settings.storeAdmin') }}" method="POST">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">{{ __('First Name') }}</label>
                                <input type="text" name="first_name" class="form-input" placeholder="{{ __('Enter first name') }}" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">{{ __('Last Name') }}</label>
                                <input type="text" name="last_name" class="form-input" placeholder="{{ __('Enter last name') }}" required>
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="form-label">{{ __('Email Address') }}</label>
                            <input type="email" name="email" class="form-input" placeholder="{{ __('Enter email address') }}" required dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                        </div>

                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">{{ __('Phone Number') }}</label>
                                <input type="text" name="phone" class="form-input" placeholder="{{ __('Enter phone number') }}" dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-row" style="margin-bottom: 30px;">
                            <div class="form-group">
                                ...
                                <div class="password-wrapper">
                                    <input type="password" name="password" id="add_admin_password" class="form-input" placeholder="{{ __('Enter strong password') }}" required dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="password-wrapper">
                                    <input type="password" name="password_confirmation" id="add_admin_password_confirm" class="form-input" placeholder="{{ __('Confirm Password') }}" required dir="ltr" style="text-align: {{ app()->getLocale() == 'ar' ? 'right' : 'left' }}">
                                    <i class="fas fa-eye-slash" onclick="togglePassword('add_admin_password_confirm', this)"></i>
                                </div>
                            </div>
                        </div>

                        <button type="submit" class="btn-save" style="background: var(--taif-green);"><i class="fas fa-plus-circle me-2"></i> {{ __('Create Admin Account') }}</button>
                    </form>
                </div>

                <div id="security" class="tab-panel">
                    <h3 class="panel-title" style="color: #c53030;"><i class="fas fa-exclamation-triangle"></i> {{ __('Account Security') }}</h3>
                    
                    <div class="danger-box">
                        <div class="danger-text">
                            <h4>{{ __('Delete Account') }}</h4>
                            <p>{{ __('Once you delete your account, there is no going back. All your associated data will be permanently removed.') }}</p>
                        </div>
                        <form id="delete-account-form" action="{{ route('admin.settings.destroy') }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="button" class="btn-delete" onclick="openDeleteModal()"><i class="fas fa-trash-alt me-2"></i> {{ __('Delete Account') }}</button>
                        </form>
                    </div>
                </div>
    ...
    <div id="logoutModal" class="custom-modal">
        <div class="modal-box">
            ...
            <h3 style="color: #2d3748; font-weight: 800; margin-bottom: 10px;">{{ __('Log Out') }}</h3>
            <p style="color: #718096; font-size: 15px; margin-bottom: 25px; font-weight: 600;">{{ __('Are you sure you want to log out of your account?') }}</p>
            <div style="display: flex; justify-content: center; gap: 10px;">
                <button type="button" class="btn-cancel" onclick="closeLogoutModal()">{{ __('Cancel') }}</button>
                <button type="button" class="btn-confirm" onclick="document.getElementById('logout-form').submit();">{{ __('Yes, Log Out') }}</button>
            </div>
        </div>
    </div>
    ...
    <div id="deleteModal" class="custom-modal">
        <div class="modal-box">
            ...
            <h3 style="color: #2d3748; font-weight: 800; margin-bottom: 10px;">{{ __('Delete Account') }}</h3>
            <p style="color: #718096; font-size: 15px; margin-bottom: 25px; font-weight: 600;">{{ __('Are you absolutely sure you want to delete your account? This action cannot be undone.') }}</p>
            <div style="display: flex; justify-content: center; gap: 10px;">
                <button type="button" class="btn-cancel" onclick="closeDeleteModal()">{{ __('Cancel') }}</button>
                <button type="button" class="btn-confirm" onclick="document.getElementById('delete-account-form').submit();">{{ __('Yes, Delete') }}</button>
            </div>
        </div>
    </div>
