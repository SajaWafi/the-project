...
class AdminAlertController extends Controller
{
    ...
    {
        // 💡 جلب التنبيه مع بيانات الطفل للتسجيل
        $alert = Alert::with('child')->findOrFail($id);

        $alert->update([
            'is_read' => true,
        ]);

        $childName = $alert->child ? $alert->child->name : 'غير معروف';
        
        // 💡 تسجيل الحركة (تعديل)
        $this->logActivity('تعديل تنبيه', "قام الأدمن بتحديد التنبيه ({$alert->title}) الخاص بالطفل ({$childName}) كمقروء");
    ...
    public function markUnread($id)
    {
        ...
        // 💡 تسجيل الحركة (تعديل)
        $this->logActivity('تعديل تنبيه', "قام الأدمن بتحديد التنبيه ({$alert->title}) الخاص بالطفل ({$childName}) كغير مقروء");
    ...
    {
        // 💡 جلب التنبيه مع بيانات الطفل لحفظ الأسماء قبل الحذف
        $alert = Alert::with('child')->findOrFail($id);
        ...
        // 💡 تسجيل الحركة (حذف)
        $this->logActivity('حذف تنبيه', "قام الأدمن بحذف التنبيه ({$alertTitle}) الخاص بالطفل ({$childName}) من المنظومة");
